...
## 알파벳인지 확인하기(isalpha)
'''
문자열의 구성이 알파벳인지에 대해서 확인하는 방법입니다. 저는 isalpha()내장 함수를 사용할 것입니다.
하지만, 단 주의가 필요합니다. 문자열에 숫자 및 공백이 포함되어 있으면 False를 리턴
'''

## 숫자인지 확인하기(isdigit)
'''
문자열의 구성이 모두 숫자인지 확인하는 메소드를 사용할 것입니다.
그 메소드는 isdigit()입니다. 위와 동일하게 이 메소드의 리턴값도 boolean형태입니다. 
'''

## 알파벳 또는 숫자인지 확인하기(isalnum) 
'''
문자열이 알파벳인지 확인하는 방법, 또는 숫자인지 확인하는 방법은 생각보다 많이 사용되지 않습니다.
다양한 이유가 있겠지만, 문자열 구성에는 생각보다 숫자와 문자가 혼용되는 경우가 매우 많기 때문입니다.
그래서 위의 이 두가지를 혼용해서 문자열이 알파벳 또는 숫자인지 확인하는 방법으로 .isalnum()를 사용합니다.
'''